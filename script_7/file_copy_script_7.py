...
VIDEOS_ORIGINAL_PATH = r"/home/kq/wjl/yxsz/CCTV-13新闻"
...
class auto_copy(object):
    ...
    def mak_data_info_4_sql(self, org_abs_path, store_abs_path):
        _t = os.path.basename(org_abs_path).split("_")
        start_date, end_date = _t[0], _t[1]
        _s = start_date.split("-")[0]
        _s1 = start_date.split("-")[1]
        start_date = _s[0:4] + "-" + _s[4:6] + "-" + _s[6:8] + " " + _s1[0:2] + ":" + _s1[2:4] + ":" + _s1[4:6]

        _s = end_date.split("-")[0]
        _s1 = end_date.split("-")[1]
        end_date = _s[0:4] + "-" + _s[4:6] + "-" + _s[6:8] + " " + _s1[0:2] + ":" + _s1[2:4] + ":" + _s1[4:6]
        # print("_____", start_date)
        # print(end_date)
        url_part = store_abs_path.split("weixing/573/")[1]
        # print("-*10", url_part)
        return start_date, end_date, url_part
    ...
    def copy_fun(self):
        ...
        print("_temp_path:  {}".format(_temp_path))
        # #先判断是不是MP4等视频格式的路径
        ...
        try:
            ...
            start_time, end_time, url_part = self.mak_data_info_4_sql(_temp_path, store_path)

            #####script_7
            # 网数据库里写字段

            task_info ={}
            task_info['date'] = start_time
            task_info['end_date'] = end_time
            task_info['chan_num'] = 3694
            task_info['video_path'] = store_path
            task_info['video_url'] = "http://192.168.1.157:8080/data/weixing/573/" + url_part
            # print( "http://192.168.1.157:3306/data/weixing/weixing/573/" + url_part )
            task_info['data_source'] = "weixing"
            task_info['chan_name'] = "cctv13"
            self.database_sql.insert_task(task_info=task_info)
            print("!!", task_info)
            print("存入sql成功!!!!!!!!")
